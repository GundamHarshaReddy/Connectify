...
export async function GET(request: NextRequest) {
  try {
    ...
    const profileIds = providers.map(p => p.profile_id).filter(Boolean)
    ...
    const processedProviders = providers.map(provider => {
      ...
      const profile = profileMap[provider.profile_id] || null
      ...
      if (profile?.full_name) {
        ...
        // If provider has a name field
        name = provider.name;
        
        // Check if name contains "Provider" with the category
        if (provider.category && name.toLowerCase().includes(`${provider.category.toLowerCase()} provider`)) {
    ...
    return NextResponse.json({ 
      ...
      debug: {
        profilesFound: profiles.data?.length || 0,
